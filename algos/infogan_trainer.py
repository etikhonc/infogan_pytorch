    ...
    def visualize_all_factors(self, epoch, it):
        ...
        if self.cuda:
            ...
            fixed_noncat = np.concatenate([
                np.tile(self.model.nonreg_latent_dist.sample_prior(10).data.cpu().numpy(), [10, 1]),  # sample from Uniform distribution
                self.model.nonreg_latent_dist.sample_prior(self.batch_size - 100).data.cpu().numpy()], axis=0)
            # 128 x 12
            fixed_cat = np.concatenate([  # 128 x 12
                np.tile(self.model.reg_latent_dist.sample_prior(10).data.cpu().numpy(), [10, 1]),
                self.model.reg_latent_dist.sample_prior(self.batch_size - 100).data.cpu().numpy()], axis=0)
        else:
            # 128 x 62
            fixed_noncat = np.concatenate([
                np.tile(self.model.nonreg_latent_dist.sample_prior(10).data.numpy(), [10, 1]),  # sample from Uniform distribution
                self.model.nonreg_latent_dist.sample_prior(self.batch_size - 100).data.numpy()], axis=0)
            # 128 x 12
            fixed_cat = np.concatenate([  # 128 x 12
                np.tile(self.model.reg_latent_dist.sample_prior(10).data.numpy(), [10, 1]),
                self.model.reg_latent_dist.sample_prior(self.batch_size - 100).data.numpy()], axis=0)
        ...
        for dist_idx, dist in enumerate(self.model.reg_latent_dist.dists):
            ...
            plt.axis('off')
            fig.savefig(os.path.join(self.log_dir, 'image_%d_%s_epoch_%03d_it_%03d.png' %
                                    (dist_idx, dist.__class__.__name__, epoch, it)), bbox_inches='tight', pad_inches=0)
